...
import os
from .forms import ConcesionForm, PersonaMoralForm, RepresentanteLegalForm, VehiculoForm
...
def dashboard_view(request):
    ...
    solicitudes = Solicitud.objects.filter(
        usuario=request.user).order_by('-creado')
...
def iniciar_proceso_view(request):
    ...
    solicitud_activa = Solicitud.objects.filter(
        usuario=request.user, estado='borrador').first()
    ...
    if not solicitud_activa:
        try:
            ...
            messages.error(
                request, "Error interno: El municipio por defecto para iniciar la solicitud no existe. Contacte a soporte.")
...
def titulo_view(request):
    ...
    solicitud_actual = Solicitud.objects.filter(
        usuario=request.user, estado='borrador').first()

    if not solicitud_actual:
        messages.error(
            request, 'No se encontró una solicitud activa. Por favor, inicie el proceso.')
        return redirect('vehiculos:dashboard')
...
def persona_fisica_view(request):
    ...
    solicitud_actual = Solicitud.objects.filter(
        usuario=request.user, estado='borrador', tipo_persona='fisica').first()
    if not solicitud_actual:
        messages.warning(
            request, 'No se encontró una solicitud activa de tipo física. Inicie el proceso.')
        return redirect('vehiculos:titulo')

    # 🟢 1. Obtener la instancia existente de PersonaFisica (si ya existe)
    persona_fisica_instance = None
    try:
        persona_fisica_instance = PersonaFisica.objects.get(
    ...
    if request.method == 'POST':
        ...
        form = PersonaFisicaForm(
            request.POST, instance=persona_fisica_instance)
    ...
    # 3. Manejo de la petición GET
        ...
        form = PersonaFisicaForm(
            instance=persona_fisica_instance, initial=session_data)
...
def persona_moral_view(request):
    """Formulario de persona moral con validación"""
    solicitud_actual = Solicitud.objects.filter(
        usuario=request.user, estado='borrador', tipo_persona='moral').first()
    if not solicitud_actual:
        messages.warning(
            request, 'No se encontró una solicitud activa de tipo moral. Inicie el proceso.')
        return redirect('vehiculos:titulo')

    if request.method == 'POST':
        form = PersonaMoralForm(request.POST)

        if form.is_valid():
            # Guardar los datos limpios en la sesión
        ...
        # Precargar datos desde la sesión si existen
        initial_data = request.session.get('persona_moral_data', {})
        form = PersonaMoralForm(initial=initial_data)

    return render(request, 'vehiculos/persona_moral.html', {'form': form})
...
def domicilio_view(request):
    ...
    if not solicitud_actual or 'persona_fisica_data' not in request.session:
        messages.warning(
            request, 'Primero debes completar los datos personales')
        return redirect('vehiculos:persona_fisica')
...
def domicilio_moral_view(request):
    ...
    if not solicitud_actual or 'persona_moral_data' not in request.session:
        messages.warning(
            request, 'Primero debes completar los datos de la empresa')
        return redirect('vehiculos:persona_moral')
...
def representante_legal_view(request):
    ...
    if not solicitud_actual or 'domicilio_moral_data' not in request.session:
        messages.warning(
            request, 'Primero debes completar el domicilio fiscal')
        return redirect('vehiculos:domicilio_moral')
...
def vehiculo_view(request):
    ...
    if request.method == 'POST':
        ...
        if tipo_persona == 'fisica':
            ...
            messages.error(
                request, 'Tipo de persona no definido en la solicitud.')
            return redirect('vehiculos:dashboard')
...
def documentos_fisica_view(request):
    ...
    if not solicitud_actual or 'vehiculo_data' not in request.session:
        messages.warning(
            request, 'Primero debes completar los datos del vehículo')
        return redirect('vehiculos:vehiculo')
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request, 'Documentos personales guardados temporalmente')
...
def documentos_moral_view(request):
    ...
    if not solicitud_actual or 'representante_data' not in request.session:
        messages.warning(
            request, 'Primero debes completar los datos del representante legal')
        return redirect('vehiculos:representante_legal')

    if request.method == 'POST':
        request.session['documentos_moral_completado'] = True
        messages.success(
            request, 'Documentos de persona moral guardados temporalmente')
        return redirect('vehiculos:documentos_vehiculo')  # Siguiente paso
...
def documentos_vehiculo_view(request):
    ...
    if not solicitud_actual:
        messages.warning(
            request, 'No se encontró una solicitud activa. Inicie el proceso.')
        return redirect('vehiculos:titulo')

    if request.method == 'POST':
        request.session['documentos_vehiculo_completado'] = True
        messages.success(
            request, 'Documentos del vehículo guardados temporalmente')
...
def finalizar_view(request):
    ...
    if not solicitud_actual:
        messages.error(
            request, 'No se encontró una solicitud en proceso. Por favor, inicie el proceso nuevamente.')
        return redirect('vehiculos:dashboard')

    if solicitud_actual.estado != 'borrador':
        # Evitar el doble envío
        messages.warning(
            request, f'La solicitud {solicitud_actual.folio} ya fue enviada.')
        context = {
    ...
    try:
        with transaction.atomic():
            ...
            if tipo_persona == 'fisica':
                ...
                domicilio_data = request.session.get(
                    'domicilio_moral_data', {})
                Domicilio.objects.update_or_create(
                    solicitud=solicitud,
                ...
                representante_data = request.session.get(
                    'representante_data', {})
            ...
            if vehiculo_data:
                marca = MarcaVehiculo.objects.get(
                    id=vehiculo_data.get('marca'))
                tipo_vehiculo = TipoVehiculo.objects.get(
                    id=vehiculo_data.get('tipo'))
                    ...
                    try:
                        ...
                        if temp_path and default_storage.exists(temp_path):
                            ...
                            final_path = default_storage.save(
                                nuevo_path, ContentFile(contenido))
            ...
            messages.success(
                request, f'¡Solicitud registrada exitosamente! Tu folio es: {solicitud.folio}')
        ...
        messages.error(
            request, f'Error al guardar la solicitud. Por favor, reintente: {str(e)}')
        return redirect('vehiculos:dashboard')
